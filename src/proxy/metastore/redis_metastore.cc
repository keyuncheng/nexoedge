...
bool RedisMetaStore::putMeta(const File &f)
{
    ...
    // generate file key to filename (with namespace)
    int nameLength = genFileKey(f.namespaceId, f.name, f.nameLength, filename);
    int vlnameLength = 0;

    // get filename prefix from filename
    std::string prefix = getFilePrefix(filename);
    int curVersion = -1;

    // find the current version of filename
    redisReply *vr = (redisReply *)redisCommand(
        _cxt, "HGET %b ver", filename, (size_t)nameLength);

    if (vr != NULL && vr->type == REDIS_REPLY_STRING && vr->len == sizeof(int)) // found version of file
    {
        memcpy(&curVersion, vr->str, sizeof(int));
    }
    else if (vr == NULL)
    { // error connecting Redis (otherwise it won't get NULL as result)
    ...
    Config &config = Config::getInstance();
    bool keepVersion = !config.overwriteFiles(); // whether need to keep version
    // put meta request has larger version
    if (keepVersion && curVersion != -1 && f.version > curVersion)
    {
        int vnameLength = genVersionedFileKey(f.namespaceId, f.name, f.nameLength, f.version - 1, vfilename); // get file key with veresion
        // TODO clone instead of put after rename
        // TODO these steps need to be an atomic transaction with HMSET,
        // otherwise metadata can be inconsistent

        // rename this file key into versionkey (version - 1) to act as a backup
        redisReply *r = (redisReply *)redisCommand(
            _cxt, "RENAME %b %b", filename, (size_t)nameLength, vfilename, (size_t)vnameLength);
        ...
        r = (redisReply *)redisCommand(
            _cxt, "HMGET %b size mtime md5 dm numC", vfilename, (size_t)vnameLength);
        ...
        // create a summary of the current version in metadata store
        std::string fsummary;
        ...
        // add file summary to a sorted set
        r = (redisReply *)redisCommand(
            _cxt, "ZADD %b %d %b", vlname, (size_t)vlnameLength, f.version - 1, fsummary.c_str(), fsummary.size());
        LOG(INFO) << "File summary of " << vlname << " version " << f.version << " is >" << fsummary.c_str() << "<";
        freeReplyObject(r);
    }

    // operate on previous versions (it means that we are operating on old
    // file verison: f.version < current version)
    if (keepVersion && curVersion != -1 && f.version < curVersion)
    {
        ...
        vr = (redisReply *)redisCommand(
            // Question: why min = f.version?
            _cxt, "ZRANGEBYSCORE %b %d %d", vlname, (size_t)vlnameLength, f.version, f.version);
        ...
        // use the versioned file key (for the latest version) as the file key
    ...
    redisAppendCommand(
        ...
        ,
        f.name, (size_t)f.nameLength, boost::uuids::to_string(f.uuid).c_str(), &f.size, (size_t)sizeof(unsigned long int), &f.numChunks, (size_t)sizeof(int)

                                                                                                                                             ,
        f.storageClass.c_str(), &f.codingMeta.coding, (size_t)sizeof(f.codingMeta.coding), &f.codingMeta.n, (size_t)sizeof(f.codingMeta.n), &f.codingMeta.k, (size_t)sizeof(f.codingMeta.k), &f.codingMeta.f, (size_t)sizeof(f.codingMeta.f), &f.codingMeta.maxChunkSize, (size_t)sizeof(f.codingMeta.maxChunkSize), &f.codingMeta.codingStateSize, (size_t)sizeof(f.codingMeta.codingStateSize), codingState, (size_t)f.codingMeta.codingStateSize

        ,
        &f.numStripes, (size_t)sizeof(int), &f.version, (size_t)sizeof(int)

                                                            ,
        &f.ctime, (size_t)sizeof(time_t), &f.atime, (size_t)sizeof(time_t), &f.mtime, (size_t)sizeof(time_t), &f.tctime, (size_t)sizeof(time_t)

                                                                                                                             ,
        &f.md5, MD5_DIGEST_LENGTH

        ,
        &f.staged.size, (size_t)sizeof(unsigned long int), f.staged.storageClass.c_str(), &f.staged.codingMeta.coding, (size_t)sizeof(f.staged.codingMeta.coding), &f.staged.codingMeta.n, (size_t)sizeof(f.staged.codingMeta.n), &f.staged.codingMeta.k, (size_t)sizeof(f.staged.codingMeta.k), &f.staged.codingMeta.f, (size_t)sizeof(f.staged.codingMeta.f), &f.staged.codingMeta.maxChunkSize, (size_t)sizeof(f.staged.codingMeta.maxChunkSize), &f.staged.mtime, (size_t)sizeof(time_t), deleted

        ,
        &numUniqueBlocks, (size_t)sizeof(size_t), &numDuplicateBlocks, (size_t)sizeof(size_t));
    ...
    for (int i = 0; i < f.numChunks; i++)
    {
        ...
        redisAppendCommand(
            _cxt, "HMSET %b %s-cid %b %s-size %b %s-md5 %b %s-bad %d", filename, (size_t)nameLength, cname, &f.containerIds[i], (size_t)sizeof(int), cname, &f.chunks[i].size, (size_t)sizeof(int), cname, f.chunks[i].md5, MD5_DIGEST_LENGTH, cname, (f.chunksCorrupted ? f.chunksCorrupted[i] : 0));
    }
    ...
    { // dedup fingerprints
        ...
        redisAppendCommand(
            _cxt, "HMSET %b %s %b%b%b%b" // logical offset, length, fingerprint, physical offset
            ,
            filename, (size_t)nameLength, bname, &it->first._offset, (size_t)sizeof(unsigned long int), &it->first._length, (size_t)sizeof(unsigned int), fp.data(), fp.size(), &it->second.second, (size_t)sizeof(int));
    }
    bid = 0;
    for (auto it = f.duplicateBlocks.begin(); it != f.duplicateBlocks.end(); it++, bid++)
    {
        ...
        redisAppendCommand(
            _cxt, "HMSET %b %s %b%b%b" // logical offset, length, fingerprint
            ,
            filename, (size_t)nameLength, bname, &it->first._offset, (size_t)sizeof(unsigned long int), &it->first._length, (size_t)sizeof(unsigned int), fp.data(), fp.size());
    }
    ...
    {
        redisAppendCommand(
            _cxt, "SET %s %b", fidKey, f.name, (size_t)f.nameLength);
        setKey += 1;
    }
...
#define check_and_convert_or_set_field(__FIELD__, __IDX__, __LEN__, __CONV__, __DEFAULT_VAL__)                                               \
    ...
    for (int i = 0; i < f.numChunks; i++)
    {
        ...
        redisAppendCommand(
            _cxt, "HMGET %b %s-cid %s-size %s-md5 %s-bad", filename, (size_t)nameLength, cname, cname, cname, cname);
    }
    ...
    { // unique blocks
        ...
        for (size_t i = 0; i < numUniqueBlocks; i++)
        {
            ...
            redisAppendCommand(
                _cxt, "HMGET %b %s", filename, (size_t)nameLength, bname);
        }
...
bool RedisMetaStore::deleteMeta(File &f)
{
    ...
    if (isVersioned && versionToDelete != -1)
    {
        ...
        vr = (redisReply *)redisCommand(
            _cxt, "ZCARD %b", vlname, (size_t)vlnameLength);
        ...
        { // delete the current version
            ...
            if (numVersions > 0)
            {
                ...
                vr = (redisReply *)redisCommand(
                    _cxt, "ZREVRANGEBYSCORE %b +inf -inf WITHSCORES LIMIT 0 1", vlname, (size_t)vlnameLength);
                ...
                vr = (redisReply *)redisCommand(
                    _cxt, "RENAME %b %b", vfilename, (size_t)vnameLength, filename, (size_t)nameLength);
        ...
        { // operates on the previous versions
            if (numVersions == 0)
            {
        ...
        if (versionToRemove != -1)
        {
            ...
            vr = (redisReply *)redisCommand(
                _cxt, "ZREMRANGEBYSCORE %b %d %d", vlname, (size_t)vlnameLength, versionToRemove, versionToRemove);
            ...
            if (curVersion != f.version)
            {
                ...
                vr = (redisReply *)redisCommand(
                    _cxt, "DEL %b", vfilename, (size_t)vnameLength);
                freeReplyObject(vr);
            }
    ...
    std::lock_guard<std::mutex> lk(_lock);
    redisReply *r = (redisReply *)redisCommand(
        _cxt, "DEL %b", filename, (size_t)nameLength);
    ...
    r = (redisReply *)redisCommand(
        _cxt, "EVAL %s 2 %s %s %b", script.c_str(), prefix.c_str(), DIR_LIST_KEY, filename, (size_t)nameLength);
...
bool RedisMetaStore::renameMeta(File &sf, File &df)
{
    ...
    std::lock_guard<std::mutex> lk(_lock);
    redisReply *r = (redisReply *)redisCommand(
        _cxt, "RENAMENX %b %b", sfname, (size_t)snameLength, dfname, (size_t)dnameLength);
    ...
    r = (redisReply *)redisCommand(
        _cxt, "SET %s %b", dfidKey, dfname, (size_t)dnameLength);
    ...
    else
    {
        ...
        r = (redisReply *)redisCommand(
            _cxt, "RENAME %b %b", dfname, (size_t)dnameLength, sfname, (size_t)snameLength);
        if (r == NULL)
        {
            redisReconnect(_cxt);
        }
    ...
    r = (redisReply *)redisCommand(
        _cxt, "HSET %b uuid %s", dfname, (size_t)dnameLength, boost::uuids::to_string(df.uuid).c_str());
    ...
    // remove file from prefix set
    r = (redisReply *)redisCommand(
        _cxt, "SREM %s %b", sprefix.c_str(), sfname, (size_t)snameLength);
    ...
    r = (redisReply *)redisCommand(
        _cxt, "SADD %s %b", dprefix.c_str(), dfname, (size_t)dnameLength);
...
bool RedisMetaStore::updateTimestamps(const File &f)
{
    ...
    redisReply *r = (redisReply *)redisCommand(
        _cxt, "HMSET %b atime %b mtime %b tctime %b", fname, (size_t)fnameLength, &f.atime, (size_t)sizeof(time_t), &f.mtime, (size_t)sizeof(time_t), &f.tctime, (size_t)sizeof(time_t));
...
int RedisMetaStore::updateChunks(const File &f, int version)
{
    ...
    redisReply *r = (redisReply *)redisCommand(
        _cxt,
        "EVAL %s 1 %s %d", script.c_str(), fname, f.version);
    int ret = 0;
    ...
    if (r != NULL && r->type != REDIS_REPLY_ERROR)
    {
        ...
        for (size_t i = 0; i < r->elements; i++)
        {
            ...
            if (withVersions && cur.version > 0)
            {
                ...
                redisReply *metar = (redisReply *)redisCommand(
                    _cxt, "ZRANGE %b 0 %d", vlname, vlnameLength, cur.version);
                if (metar == NULL || metar->type != REDIS_REPLY_ARRAY || metar->elements < 1)
                {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        for (size_t vi = 0; vi < total; vi++)
                        {
                            ...
                            {
                                if (ofs[0] != '-')
                                { // if the field is available (not blanked)
                                    switch (vj)
                                    {
...
unsigned int RedisMetaStore::getFolderList(std::vector<std::string> &list, unsigned char namespaceId, std::string prefix, bool skipSubfolders)
{
    ...
    {
        r = (redisReply *)redisCommand(
            _cxt, "SSCAN %s %s MATCH %s", DIR_LIST_KEY, cursor.c_str(), pattern.c_str());
...
int RedisMetaStore::getFilesToRepair(int numFiles, File files[])
{
    ...
    {
        LOG(ERROR) << "Failed to get files to repair, " << (r == NULL ? "failed to get a reply" : " reply type = ") << (r != NULL ? r->type : 0) << " " << (r != NULL && r->type == REDIS_REPLY_ERROR ? r->str : "(NIL)");
    }
...
int RedisMetaStore::getFilesPendingWriteToCloud(int numFiles, File files[])
{
    ...
    redisReply *r = (redisReply *)redisCommand(
        _cxt,
        "SCARD %s_copy", FILE_PENDING_WRITE_KEY);
    ...
    if (empty)
    {
        r = (redisReply *)redisCommand(
            _cxt,
            "SDIFFSTORE %s_copy %s %s_not_exists", FILE_PENDING_WRITE_KEY, FILE_PENDING_WRITE_KEY, FILE_PENDING_WRITE_KEY);
    ...
    r = (redisReply *)redisCommand(
        _cxt,
        "SPOP %s_copy", FILE_PENDING_WRITE_KEY);
    ...
    r = (redisReply *)redisCommand(
        _cxt,
        "SMOVE %s %s %s", FILE_PENDING_WRITE_KEY, FILE_PENDING_WRITE_COMP_KEY, key.c_str());
...
bool RedisMetaStore::updateFileStatus(const File &file)
{
    ...
    if (file.status == FileStatus::PART_BG_TASK_COMPLETED)
    {
        ...
        r = (redisReply *)redisCommand(
            _cxt,
            "EVAL %s 1 %s %s", script.c_str(), BG_TASK_PENDING_KEY, filename, nameLength);
    ...
    else if (file.status == FileStatus::BG_TASK_PENDING)
    {
        ...
        r = (redisReply *)redisCommand(
            _cxt,
            "ZINCRBY %s 1 %b", BG_TASK_PENDING_KEY, filename, (size_t)nameLength);
    ...
    else if (file.status == FileStatus::ALL_BG_TASKS_COMPLETED)
    {
        r = (redisReply *)redisCommand(
            _cxt,
            "ZREM %s %b", BG_TASK_PENDING_KEY, filename, (size_t)nameLength);
        ret = r != NULL && r->type == REDIS_REPLY_INTEGER && r->integer == 1;
        if (ret)
        {
    ...
    r = (redisReply *)redisCommand(
        _cxt,
        "HSET %b tctime %b", filename, (size_t)nameLength, &tctime, (size_t)sizeof(tctime));
...
bool RedisMetaStore::getNextFileForTaskCheck(File &file)
{
    ...
    redisReply *r = (redisReply *)redisCommand(
        _cxt,
        "ZSCAN %s %s COUNT 1", BG_TASK_PENDING_KEY, _taskScanIt.c_str());
...
bool RedisMetaStore::addChunkToJournal(const File &file, const Chunk &chunk, int containerId, bool isWrite)
{
    ...
    do
    {
        ...
        r = (redisReply *)redisCommand(
            _cxt, "HSCAN %b %d MATCH %s-op*", key, (size_t)keyLength, cursor, cname);
        ...
        for (size_t ei = 0; r->elements >= 2 && ei < r->element[1]->elements; ei++)
        {
            ...
            if (ei % 2 == 1 && strncmp(value->str, "w", 1) == 0)
            {
                ...
                redisAppendCommand(
                    _cxt, "HSET %b %b %s", key, (size_t)keyLength, preValue->str, preValue->len, "d");
    ...
    ";
    r = (redisReply *)redisCommand(
        _cxt, "EVAL %s 2 %b %s %s-size-%d %b %s-md5-%d %b %s-op-%d %s %s-status-%d %s %b", script.c_str(), key, (size_t)keyLength /* KEYS[1] */
        ,
        JL_LIST_KEY /* KEYS[2] */
        ,
        cname, containerId, &chunk.size, sizeof(int), cname, containerId, chunk.md5, (size_t)MD5_DIGEST_LENGTH, cname, containerId, opType, cname, containerId, status, filename, (size_t)nameLength /* ARGV[9] */
    );
    if (r == NULL || r->type != REDIS_REPLY_INTEGER || r->integer == -1)
    {
        freeReplyObject(r);
...
bool RedisMetaStore::updateChunkInJournal(const File &file, const Chunk &chunk, bool isWrite, bool deleteRecord, int containerId)
{
    ...
    if (deleteRecord)
    {
        ...
        std::string script =
            ...
            return 2;";
        r = (redisReply *)redisCommand(
            _cxt, "EVAL %s 3 %b %s %b %s-size-%d %s-md5-%d %s-op-%d %s-status-%d", script.c_str(), key, (size_t)keyLength, JL_LIST_KEY, filename, (size_t)nameLength, cname, containerId, cname, containerId, cname, containerId, cname, containerId);
    ...
    else
    {
        ...
        std::string script =
            ...
            ";";
        r = (redisReply *)redisCommand(
            _cxt, "EVAL %s 1 %b %s-op-%d %s-status-%d %s %s", script.c_str(), key, (size_t)keyLength, cname, containerId, cname, containerId, opType, status);
...
void RedisMetaStore::getFileJournal(const FileInfo &file, std::vector<std::tuple<Chunk, int /* container id*/, bool /* isWrite */, bool /* isPre */>> &records)
{
    ...
    redisReply *r = (redisReply *)redisCommand(
        _cxt, "HGETALL %b", key, (size_t)keyLength);
    if (r == NULL)
    {
    ...
    for (size_t i = 0; i < r->elements; i++)
    {
        ...
        else if (chunkId != INVALID_CHUNK_ID && containerId != INVALID_CONTAINER_ID && !type.empty())
        {
            ...
            if (listIndexIt != chunk2listIndex.end())
            {
                ...
                { // whether the record is pre-operation
                    std::get<3>(record) = strncmp(r->element[i]->str, "pre", 3) == 0;
                }
                else if (type.compare("op") == 0)
...
bool RedisMetaStore::fileHasJournal(const File &file)
{
    ...
    redisReply *r = (redisReply *)redisCommand(
        _cxt, "SISMEMBER %s %b", JL_LIST_KEY, filename, (size_t)nameLength);
...
{
    const char *slash = strrchr(name, '/'), *us = strchr(name, '_'); // locate the namespace char
